...
/**
 * Signal-sources bento tile — the radial hub, shrunk from hero to instrument.
 * Spokes are real buttons: click (or Enter) filters the live feed by channel;
 * click again to clear. Hover/focus previews the channel in the hub centre.
 */
export function RadialHubTile() {
  ...
  return (
    <div className="glass shrink-0 rounded-xl px-4 pb-1 pt-3">
      <div className="flex items-baseline justify-between">
        <span className="text-[13px] font-semibold text-ink">Signal sources</span>
        <span className="text-[11px] text-faint">
          {sourceFilter === "all" ? "this week" : "filtering feed"}
        </span>
      </div>

      <svg viewBox="0 0 360 240" className="block w-full overflow-visible" role="group" aria-label="Intake channels — activate a channel to filter the live feed">
