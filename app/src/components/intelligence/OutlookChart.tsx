    ...
    <section className="mx-auto max-w-[1440px] px-10 pt-10">
      ...
      <div className="overflow-hidden rounded-xl border border-line bg-surface px-8 py-7">
        <svg viewBox="0 0 1280 300" className="block h-[300px] w-full" role="img" aria-label="Line chart projecting weekly signal volume for public health, water, and infrastructure over the next 12 weeks. Public health peaks near 220 signals around Diwali in week 45.">
          {/* Axes */}
          <line x1={60} y1={30} x2={60} y2={260} stroke="#EDE7D7" strokeWidth={1} />
          <line x1={60} y1={260} x2={1240} y2={260} stroke="#EDE7D7" strokeWidth={1} />

          {/* Y labels + horizontal grid */}
          {C.yLabels.map((l) => (
            <text key={l.y} x={52} y={l.y} textAnchor="end" fontSize={10} fill="#A69C86">
              {l.label}
            </text>
          ))}
          {[30, 90, 150, 210].map((y) => (
            <line key={y} x1={60} y1={y} x2={1240} y2={y} stroke="#F6F2EA" strokeWidth={1} strokeDasharray="2,3" />
          ))}

          {/* Actual/forecast split */}
          <line x1={C.nowX} y1={30} x2={C.nowX} y2={260} stroke="#CDC5B4" strokeWidth={1} strokeDasharray="4,4" />
          <rect x={C.nowX} y={30} width={1240 - C.nowX} height={230} fill="#FBF8F2" opacity={0.4} />
          <text x={C.nowX + 4} y={45} fontSize={10} fill="#A69C86">
            Forecast →
          </text>
          <text x={C.nowX - 4} y={45} textAnchor="end" fontSize={10} fill="#A69C86">
            ← Actual
          </text>
          ...
          <g className="chart-fade" style={{ animationDelay: "1.5s" }}>
            ...
            <circle cx={C.peak.x} cy={C.peak.y} r={4} fill="#C15A15" stroke="#FFFFFF" strokeWidth={2} />
            <text x={C.peak.x + 10} y={C.peak.y - 8} fontSize={11} fill="#14192A" fontWeight={500}>
              {C.peak.label}
            </text>
          </g>
        ...
        {/* Chart footer */}
        <div className="mt-[18px] flex items-center justify-between border-t border-line-faint pt-4 text-xs text-muted">
          <div>
            {C.footer.trainedOn} ·{" "}
